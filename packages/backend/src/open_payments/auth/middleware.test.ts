import { ActiveTokenInfo } from 'auth'
...
// TODO: replace with http-signature-utils
import { generateSigHeaders } from 'auth/src/tests/signature'
...
describe('HTTP Signature Middleware', (): void => {
  ...
  `('$description', ({ body }): void => {
    beforeEach(async (): Promise<void> => {
      ...
      const requestSignatureHeaders = await generateSigHeaders({
        privateKey,
        keyId,
        url,
        method,
      ...
      ctx = createContext<HttpSigContext>({
        headers: {
          ...
          'Content-Length': requestSignatureHeaders.contentLength,
          'Content-Type': requestSignatureHeaders.contentType
        },
        method,
        body,
        url
